...
static int collect_mean_temp(const domain_t *domain, const temperature_t *temperature, statistics_t *statistics){
  ...
  /* ! temp and its square are added ! 6 ! */
  for(int j = 1; j <= jsize; j++){
    for(int i = 0; i <= isize+1; i++){
      TEMP1(i, j) += pow(TEMP(i, j), 1.);
      TEMP2(i, j) += pow(TEMP(i, j), 2.);
    }
  }
  return 0;
}
...
int statistics_collect(const domain_t *domain, const double time, const fluid_t *fluid, const temperature_t *temperature, const interface_t *interface, statistics_t *statistics){
  if(!is_scheduled){
    ...
    /* ! collect temporally-averaged quantities ! 3 ! */
