...
int save_interface(const char dirname[], const domain_t *domain, const interface_t *interface){
  const int glisize = domain->glsizes[0];
  const int gljsize = domain->glsizes[1];
  const int   isize = domain->mysizes[0];
  const int   jsize = domain->mysizes[1];
  ...
  // vof: [0:isize+1] x [1:jsize]
  {
    ...
    fileio_w_nd_parallel(dirname, "vof", NDIMS, glsizes, mysizes, offsets, buf);
    common_free(buf);
  }
  return 0;
}
 ...
 * @return                : error code
 */
int save(const domain_t *domain, const int step, const double time, const fluid_t *fluid, const temperature_t *temperature, const interface_t *interface){
  if(!is_scheduled){
    ...
    save_interface(dirname, domain, interface);
    common_free(dirname);
    save_next += save_rate;
  }
  return 0;
