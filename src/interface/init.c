...
static int allocate(
  ...
  if(0 != array.prepare(domain, FLXX_NADDS, sizeof(double), &interface->flxx)) return 1;
  if(0 != array.prepare(domain, FLXY_NADDS, sizeof(double), &interface->flxy)) return 1;
  if(0 != array.prepare(domain, FLXZ_NADDS, sizeof(double), &interface->flxz)) return 1;
...
static void report(
  ...
  if(root == myrank){
    ...
    fprintf(stream, "\tsurface tension: % .7e\n", interface->tension);
    fflush(stream);
  }
}
...
int interface_init(
  ...
  // compute surface tension coefficient | 3
  double We = 0.;
  if(0 != config.get_double("We", &We)) return 1;
  interface->tension = 1. / We;
